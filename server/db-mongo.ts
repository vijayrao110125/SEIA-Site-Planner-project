...
async function getSessionsCollection() {
  ...
  try {
    ...
    const old = idx.find((i) => i?.unique && JSON.stringify(i?.key) === JSON.stringify({ nameNormalized: 1 }));
...
export async function updateUserName(id, name) {
  const now = new Date().toISOString();
  const col = await getUsersCollection();
  const res = await col.updateOne({ id }, { $set: { name: typeof name === "string" ? name : null, updatedAt: now } });
...
export async function listSessions(userId) {
  const col = await getSessionsCollection();
  return col
    .find({ userId }, { projection: { _id: 0, id: 1, name: 1, createdAt: 1, updatedAt: 1 } })
    .sort({ updatedAt: -1 })
    .toArray();
}
...
export async function updateSession(userId, id, { name, payload }) {
  ...
  const nameNormalized = normalizeName(name);
  if (nameNormalized) {
    const existing = await col.findOne({ userId, nameNormalized, id: { $ne: id } }, { projection: { _id: 1 } });
    if (existing) {
      const err = new Error("Session name already exists");
      (err as any).code = 11000;
      throw err;
