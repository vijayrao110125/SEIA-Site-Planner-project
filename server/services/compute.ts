...
function clampNonNegInt(v: unknown) {
  const n = Number(v);
  if (!Number.isFinite(n)) return 0;
  return Math.max(0, Math.floor(n));
}

function normalizeCounts(counts: Record<string, unknown>) {
...
function computeTotals(counts: Record<string, number>) {
  let totalCost = 0;
  let totalEnergyMWh = 0;

  for (const [type, c] of Object.entries(counts)) {
    const def = CATALOG[type as keyof typeof CATALOG];
...
function expandRectangles(counts: Record<string, number>) {
  // each instance becomes a rect
  const rects: Array<{ id: string; type: string; w: number; d: number }> = [];
  let idx = 1;
  for (const [type, c] of Object.entries(counts)) {
    const def = CATALOG[type as keyof typeof CATALOG];
    for (let i = 0; i < c; i++) {
...
function packLayout(rects: Array<{ id: string; type: string; w: number; d: number }>) {
  // Greedy row packing (shelf algorithm)
  const placements: Array<{ id: string; type: string; w: number; d: number; x: number; y: number }> = [];
  let x = 0;
  let y = 0;
  let rowHeight = 0;
  let usedWidth = 0;
...
export function computeAll(rawCounts: Record<string, unknown>) {
