...
import SessionsBar from "../components/sessions/SessionsBar";
...
export default function DashboardScreen({
  ...
  catalog: any;
  counts: any;
  onCountChange: (type: any, value: string) => void;
  ...
  return (
    <>
      <div className="foreground relative z-10">
          ...
          <div className="mx-auto max-w-6xl px-4 py-4 flex flex-col lg:flex-row lg:items-start justify-between gap-4">
            <div className="lg:max-w-xl">
              ...
              <div className="text-xs text-zinc-500 dark:text-[#AAB6C9] mt-1">
                Signed in as{" "}
                <span className="font-medium text-zinc-700 dark:text-[#D5DEEB]">{user.name || user.email}</span>
              </div>
            </div>
            ...
            <div className="rounded-2xl bg-white dark:bg-[#111C2D] shadow-sm border border-zinc-200 dark:border-[#26334A] p-4">
              ...
              <div className="space-y-3">
                {BATTERY_TYPES.map((type) => (
                  <DeviceCard key={type} type={type} def={catalog[type]} value={counts[type]} onChange={(v) => onCountChange(type, v)} />
                ))}
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <div className="text-xs text-zinc-500 dark:text-[#AAB6C9] mt-0.5">
                        {catalog["Transformer"].w}ft × {catalog["Transformer"].d}ft • {catalog["Transformer"].energyMWh} MWh • $
                        {catalog["Transformer"].cost.toLocaleString()}
                        {catalog["Transformer"].release ? ` • ${catalog["Transformer"].release}` : ""}
                      </div>
                      <div className="text-xs text-zinc-500 dark:text-[#AAB6C9]">Rule: 1 transformer per 2 industrial batteries</div>
                    </div>
        ...
        <footer className="py-8 text-center text-xs text-zinc-500 dark:text-[#8090AA]">Frontend and API running</footer>
      </div>
      {children}
    </>
  );
