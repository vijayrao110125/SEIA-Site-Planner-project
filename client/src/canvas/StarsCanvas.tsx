...
function randomInSphereFloat32(count: number, radius: number) {
...
function Stars({
  ...
  return (
    <group rotation={[0, 0, Math.PI / 4]}>
      <Points ref={ref} positions={sphere} stride={3} frustumCulled position={[0, 0, -0.8]} {...props}>
        <PointMaterial
          transparent
          color={color}
...
export default function StarsCanvas({ theme = "light" }: { theme?: "light" | "dark" }) {
  ...
  return (
    <div className="stars-canvas" style={{ opacity: layerOpacity }}>
      <Canvas dpr={[1, 1.5]} gl={{ antialias: false, alpha: true }} camera={{ position: [0, 0, 1] }}>
